...
class BeadsToCody {
  ...
  updateBacklogWithProgress() {
    ...
    for (const [version, stats] of Object.entries(versionStats)) {
      ...
      const progressSectionIndex = lines.findIndex((line, idx) => 
        idx > versionHeaderIndex && 
...
**Risk Assessment:** ${riskText}`;
          ...
          const hasProgressSection = nextLines.some(line => line.includes('Implementation Progress'));
        ...
        this.updates.push(`v${version}: ${stats.completionRate}% complete, ${stats.risk.toLowerCase()}`);
      }
    }
