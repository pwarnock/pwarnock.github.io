...
Create a **pre-commit/pre-push hook** that validates all interactive components
have proper analytics `data-event` attributes. This ensures developers can't
accidentally commit untracked interactions.
...
- Related: `scripts/build-path.sh` (may integrate with existing path-based
  build)
...
Created comprehensive Feature Development Checklist that makes analytics
mandatory. This hook enforces it.
...
**Recommendation**: Bash + ripgrep for speed. Hook should complete in <2
seconds.
...
ℹ️  See: docs/development/FEATURE_DEVELOPMENT_CHECKLIST.md
```
...
Document this in hook output: "Use `git push --no-verify` to skip (not
recommended)"
...
1. ✅ Detect all interactive HTML elements
2. ✅ Verify `data-event` and `data-event-label` attributes exist
3. ✅ Recognize `button.html` component (no double-checking)
4. ✅ Report violations with file:line numbers
5. ✅ Execute in <2 seconds on typical 20-file change
6. ✅ Prevent push if violations found
7. ✅ Document exceptions clearly
8. ✅ Provide help text (which attributes, where to learn more)
...
```html
<!-- ❌ FAIL: No data-event attribute -->
<button class="btn btn-primary">Download</button>

<!-- ❌ FAIL: data-event but missing label -->
<button data-event="download">Download</button>

<!-- ❌ FAIL: External link, no tracking -->
<a href="https://example.com">Visit Site</a>

<!-- ✅ PASS: Has both attributes -->
<button data-event="button_click" data-event-label="Download">Download</button>

<!-- ✅ PASS: Generated by button component -->
{{ partial "components/button.html" (dict "trackingEvent" "download") }}

<!-- ✅ PASS: Internal navigation, exception -->
<a href="/blog/">Read Blog</a>

<!-- ✅ PASS: Disabled, exception -->
<button disabled>Not Available</button>

<!-- ✅ PASS: Marked no-track -->
...
- Hook should be non-blocking for team productivity (clear error + bypass
  available)
