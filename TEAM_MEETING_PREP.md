...
**Gap**: When we release, we can't trace which work implemented which features. Release notes are guesswork.
...
2. Create issues: bd-47 (restructure), bd-48 (keyboard), bd-49 (mobile), bd-50 (a11y)
...
- "What if something is blocked?" → Dependencies prevent it from showing in `bd ready`
...
- "What if we hate this?" → We gather feedback after one version, adjust or move to Phase 2
...
**Q: Is this mandatory?**  
A: Yes. For one version. After that, we'll evaluate Phase 2. For now, this is how we work.
...
A: We want to understand the workflow first. After 1-2 versions, we'll automate syncing.
