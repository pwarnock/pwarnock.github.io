...
1.  **Develop:** Make changes and commit.
2.  **Bump:** When ready to release, update `package.json`:
    ```bash
    bun pm version patch  # or minor/major
    ```
3.  **Sync:** The build process (`bun run build`) automatically generates
    `data/version.toml` from `package.json`. No manual sync to `hugo.toml` is
    required.
4.  **Release:** Run the release script:
    ```bash
    ./scripts/release.sh pre   # For RC
    ./scripts/release.sh post  # For Production
    ```
...
#### data/version.toml (Generated)

This file is automatically generated during build
(`scripts/generate-version.js`) and should not be edited manually.
...
The `package.json` version is the single source of truth. Development builds
will display the current version plus the git hash (e.g., `v0.19.4 (2cdcb75)`).
...
The release process is automated via `scripts/release.sh`.
