...
**Purpose**: Establish a consistent process for adding tests when developing new features.
...
test.describe('Feature Name (pw-XXX)', () => {
  test('specific user action leads to expected result @e2e', async ({ page }) => {
    await page.goto('/');
...
- Test across all browsers (tests run on Chromium, Firefox, WebKit automatically)
...
- **Example Tests**: `/tests/e2e-journeys.spec.ts`, `/tests/analytics-tracking.spec.ts`
