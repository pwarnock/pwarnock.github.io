...
This guide documents the user experience (UX) and operational workflows for the
self-enforcing release process. It complements the technical architecture
document with practical examples, troubleshooting, and best practices.
...
**Symptoms**: Release request created 10+ minutes ago, still shows `pending`
status.
...
3. If `failed`:
   ...
   - Check deploy completed: `git log --oneline | head -5` (should show version
     bump)
...
- **[Deployment Guide](./DEPLOYMENT.md)** — Environment-specific deployment
  details
- **[Release Controller Source](.github/workflows/release-controller.yml)** —
