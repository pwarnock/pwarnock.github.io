...
- **After:** Theme-aware colors with opacity variations
  - Text: `oklch(var(--p))`
  - Hover: `oklch(var(--p) / 0.8)`
  - Border: `2px solid oklch(var(--p))`
  - Focus: Dynamic `oklch(var(--p))`
...
- **After:** Theme-aware styling
  - Background: `oklch(var(--bc) / 0.05)` with subtle border
  - Text: `oklch(var(--bc))`
...
- **Before:** Hardcoded neutral colors
- **After:** Theme-aware styling
  - Borders: `oklch(var(--bc) / 0.1)`
  - Header: `oklch(var(--b2))` with `oklch(var(--bc))`
  - Even rows: `oklch(var(--bc) / 0.02)`
...
- **Impact:** Light theme users won't be affected; dark theme users may see white color in browser UI
...
**Version:** 0.13.1 (pending)
**Category:** Accessibility & Theme Support
**Summary:** Fixed font color contrast issues across all 28 DaisyUI themes by replacing hardcoded colors with semantic DaisyUI CSS variables.
